    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 theme-transition">
      ...
      >
        <div className="space-y-2">
          <div className="flex items-center space-x-2.5 flex-wrap">
            <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Employer Command Center</h1>
            {isVerified ? (
              <span className="inline-flex items-center space-x-1 text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/20">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500" />
                <span>Statutory Verified Entity</span>
              </span>
            ) : (
              <span className="inline-flex items-center space-x-1 text-xs font-bold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-0.5 rounded-full border border-amber-500/20">
                <AlertTriangle className="w-3.5 h-3.5 text-amber-500" />
                <span>Verification Pending</span>
              </span>
            )}
          </div>
          <p className="text-xs text-slate-500 dark:text-slate-400">Track candidates, manage verified openings, and audit employer trust metrics.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {!isVerified && (
            <Link
              to="/employer/verify"
              className="px-4 py-2.5 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs transition flex items-center space-x-1.5 shadow-sm"
            >
          ...
          <Link
            ...
            className="px-4 py-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white font-bold text-xs border border-slate-200 dark:border-slate-700 transition flex items-center space-x-1.5"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <motion.div whileHover={{ y: -2 }} className="p-5 rounded-3xl bg-white/90 dark:bg-slate-900/85 border border-slate-200 dark:border-slate-800 flex items-center justify-between shadow-sm">
          ...
          <AlertTriangle className={`w-8 h-8 ${overview.fraudReportsCount > 0 ? 'text-rose-500/30' : 'text-slate-400/30'}`} />
        </motion.div>
      </div>
      ...
      <div className="p-6 rounded-3xl bg-white/90 dark:bg-slate-900/85 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {Object.entries(funnel).map(([statusKey, count]) => (
            <div key={statusKey} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800/80 text-center">
              <span className="text-[10px] font-mono uppercase text-slate-500 dark:text-slate-400 font-bold">{statusKey}</span>
              <div className="text-xl font-bold font-mono text-slate-900 dark:text-white mt-1">{count}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white/90 dark:bg-slate-900/85 border border-slate-200 dark:border-slate-800 shadow-sm space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Review resumes, contact details, and advance applicants through hiring stages.</p>
          </div>
          {recentJobs.length > 0 && (
            <select
              value={selectedJobId}
              onChange={(e) => setSelectedJobId(e.target.value)}
              className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs rounded-2xl px-3 py-2 focus:outline-none font-semibold"
            >
        ...
        {applicants.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
              <thead className="bg-slate-50 dark:bg-slate-950/80 text-slate-500 dark:text-slate-400 uppercase font-mono text-[10px] border-b border-slate-200 dark:border-slate-800">
                <tr>
                ...
                {applicants.map((app) => (
                  <tr key={app._id} className="hover:bg-slate-50 dark:hover:bg-slate-900/40 transition">
                    <td className="py-3.5 px-4">
                      <div className="font-bold text-slate-900 dark:text-white">{app.candidate?.name || 'Applicant'}</div>
                    ...
                    <td className="py-3.5 px-4">
                      {app.resumeUrl ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="text-emerald-600 dark:text-emerald-400 hover:underline font-mono inline-flex items-center space-x-1 bg-emerald-50 dark:bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20 font-bold"
                        >
                    ...
                    <td className="py-3.5 px-4">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase font-mono border ${
                        app.status === 'hired' ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-400 border-emerald-500/40' :
                        app.status === 'rejected' ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-800 dark:text-rose-400 border-rose-500/40' :
                        app.status === 'interview' ? 'bg-blue-100 dark:bg-blue-500/20 text-blue-800 dark:text-blue-400 border-blue-500/40' :
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <select
                        ...
                        className="bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 text-xs rounded-xl px-2.5 py-1 focus:outline-none font-semibold"
                      >
          ...
          <div className="text-center py-16 bg-slate-50 dark:bg-slate-950/40 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-2">
            <Users className="w-10 h-10 text-slate-400 mx-auto" />
            <h4 className="text-sm font-semibold text-slate-900 dark:text-white">No applicants received yet</h4>
            <p className="text-xs text-slate-500 dark:text-slate-400">As candidates discover your verified listings, their submissions will populate here.</p>
          </div>
        )}
      </div>
    </div>
